<?php
/**
 * The Template for displaying product archives with multi-select filter sidebar.
 * 
...
if (is_eurosuit_page()) {
  $hide = 'bride-co';
  $hide_technical ='technical-spec';
}else{
  $hide = 'eurosuit';
  $hide_technical ='';
}

// Get the current orderby from URL or set default
$current_orderby = isset($_GET['orderby']) ? sanitize_text_field($_GET['orderby']) : 'date';
?>

<!-- Add meta tags with current category and orderby for JS to access -->
...
<div class="sidebar" id="sidebar">
    ...
    <ul class="sidebar-menu">
        ...
        // Get products for the current view with consistent ordering
        $products_args = array(
            'status' => 'publish',
            'limit' => -1,
        ...
        // Apply consistent ordering based on current sort option
    ...
    default:
        ...
        if (!empty($attribute_taxonomies)) {
            foreach ($attribute_taxonomies as $attribute) {
                ...
                if ((!isset($used_attributes[$attribute->attribute_name])) || (($attribute->attribute_name==='brand' && is_eurosuit_page()))) {
                  continue;
              }
                ...
                if (!empty($terms) && !is_wp_error($terms) && ($hide_technical!==($attribute->attribute_name))) {
...
<div class="main-content">
  ...
  <div class="sort-dropdown">
    <div class="sort-dropdown-button">
      // TO THIS:
<?php 
$orderby_text = 'Newest'; // Default to Newest instead of Most Popular
...
?><span>▾</span>
  ...
  if ($has_active_filters && !empty($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'archive-product.php') === false) {
    // Or any check that confirms the user actively applied a filter this session
